...
extension ServerUpgrader {
    func upgradeFromSchema0(callback: @escaping ClosureBool) {
        ...
        api.fetchGroups(callbackOn: queue) { [weak self] (success, allGroupData) in
            let log = Logger.self
            ...
            self.api.getSchema0Data(callbackOn: queue) { [weak self] (success, data) in
                guard let self = self, success else {
                ...
                do {
                    for asset in data {
                        ...
                        if let tripKeyStringEnc = asset["tripkey"] as? String {
                            ...
                            let tripKeyString = try self.userKey.decrypt(tripKeyStringEnc, signedBy: self.userKey)
                            ...
                            let newAssetKeyStringEnc = self.userKey.encrypt(assetKeyString, signed: self.userKey)
                            ...
                            if let assetKeyStringEnc = asset["key"] as? String {
                                ...
                                let assetKeyString = try self.userKey.decrypt(assetKeyStringEnc, signedBy: self.userKey)
                                assetKeyOpt = try CryptoPrivateKey(key: assetKeyString, password: nil, for: .asset)
                                ...
                                let groupKeyString = try self.userKey.decrypt(groupKeyStringEnc, signedBy: self.userKey)
                        ...
                        if asset["md5"] is NSNull {
                            ...
                            dispatchGroup.enter()
                            self.dataManager.downloadFile(at: originalURL, to: tempURL, priority: .high) { [weak self] success in
                ...
                dispatchGroup.notify(queue: queue) {
                    guard !failed else { DispatchQueue.main.async { callback(false) }; return }
                    self.api.patchSchema0Data(assetKeys: assetKeys, assetMD5s: assetMD5s, callbackOn: queue) { success in
